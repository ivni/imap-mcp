...
- `imap_mcp/imap_client.py` — IMAP operations (connect, search, fetch, move, delete, threading). `search_newest(criteria, folder, limit)` returns `(newest-first UIDs capped at limit, total match count)` — it prefers the server's `SORT` extension (exact by Date, capability-detected — never a hostname check) and falls back to plain SEARCH ordered by UID-descending; this is what lets `search_emails` fetch only the page it needs instead of every match
- `imap_mcp/tools.py` — MCP tool registrations with ToolAnnotations (3 tiers: read-only, write non-destructive, write destructive); all tools have title + annotations. `search_emails` fetches summaries for only the newest `offset + limit` messages **per folder** (via `search_newest`) rather than every match — without this bound a single large folder makes `fetch_summaries` download hundreds of envelopes (tens of seconds) and blow the client's tool-call timeout, and the old `MAX_FETCH_UIDS` truncation kept the *lowest* UIDs so it even returned the *oldest* mail. With `folder=None` it fans out over all allowed folders under a wall-clock budget (`IMAP_MCP_SEARCH_BUDGET`, default 60s); when the budget is hit it stops, flags the response `truncated`, and lists `folders_searched`/`folders_skipped` rather than hanging past the client's tool-call timeout (a folder whose search raises is reported in `folders_errored`, and `total` counts only fully-searched folders, so coverage is never silently partial)
...
- HTTP transport sets `json_response=True` by default (env opt-out `IMAP_MCP_HTTP_JSON_RESPONSE=false`): tool results return as one JSON HTTP body, not an SSE stream. This sidesteps a Codex streamable-HTTP client bug (openai/codex#12869, #13969) where, once the SSE channel goes stale (401/404 "Session not found" — e.g. an OIDC access token expiring on the long-lived `GET /mcp` stream), every later `tools/call` hangs until the client's ~300s deadline. Spec-compliant clients negotiate either form via `Accept`, so defaulting on is safe (Claude works with both). Unrelated to response size; keep this default on unless a client needs SSE
