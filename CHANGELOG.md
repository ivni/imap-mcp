...
- The `search_emails` tool now returns a structured object instead of a
  serialized JSON string, so MCP clients receive it as `structuredContent`
  (with a JSON text fallback) rather than having to parse a string. All return
  paths — success and every input-validation error — now share one shape
  (`total`/`offset`/`limit`/`results`, plus an `error` key on bad input);
  previously some errors were bare strings while others were JSON, so callers
  could not parse errors uniformly. Clients that read the text block are
  unaffected; the content is still JSON.
